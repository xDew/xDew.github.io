...
options.randomFav = false;
options.stopmove = false;
options.seconds = false;
options.cycleTheme = false;
options.disableweather = false;
options.disablemenu = false;
options.resetls = false;
options.disablewall = false;
options.disableoverlay = false;
options.timedChanges = false;
options.aniWeather = false;
options.aniBirds = false;
options.setsbwall = false;
options.disableboxblur = false;
options.weatherrefresh = 60000 * 30;
options.timeInterval = 3600000;
options.dropColor = 'rgba(255, 255, 255, 0.5)';
...
// (function () {
//     var truthy = ['noHide', 'twentyfour', 'randomFav', 'celsius', 'stopmove', 'seconds', 'cycleTheme', 'disableweather', 'disablemenu', 'resetls', 'disablewall', 'disableoverlay', 'timedChanges', 'aniWeather', 'aniBirds', 'setsbwall', 'disableboxblur'],
//         settings = cycript.readDict('/private/var/mobile/Library/Preferences/com.junesiphone.lockplus.plist') || {},
//         i;
//     for (i = 0; i < truthy.length; i++) {
//         if (Number(settings[truthy[i]]) === 1) {
//             options[truthy[i]] = true;
//         } else {
//             options[truthy[i]] = false;
//         }
//     }
//     options.weatherrefresh = (60000 * Math.round(settings.weather)) || 60000 * 30;
//     options.timeInterval = settings.timeInterval || 3600000;
//     options.dropColor = settings.rainColor || 'rgba(255, 255, 255, 0.5)';
//     options.aniBirds = !!+settings.aniBirds || false;
//     options.aniWeather = !!+settings.aniWeather || false;
//     options.clockrefresh = 1000;
//     options.languages = (window.navigator.language.length >= 2) ? window.navigator.language.split('-')[0] : 'en';
// }());
